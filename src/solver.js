...
export function get_input_item_variable_key(item_key) {
    return `input_item_variable__${item_key}`;
}

export function get_item_constraints(item_keys, preferences) {
    /**
    ...
    for(let item_key of item_keys) {
        let constraint_value = preferences.outputs.get(item_key) || 0.0;
        item_constraints.set(item_key, constraint_value);
    }
    return item_constraints;
}

export function get_item_variable_coefficients(item_keys, recipe_variables, parsed_data, preferences) {
    /**
    ...
    for(let item_key of item_keys) {
        item_variable_coefficients.set(item_key, new Map());
    }
    ...
    for(let recipe_variable of recipe_variables) {
        ...
        for(let ingredient of recipe_data.ingredients) {
            let ingredient_item_key = get_item_key(ingredient.name, recipe_variable.ingredient_quality);
            let ingredient_item_coefficients = item_variable_coefficients.get(ingredient_item_key);
        ...
        for(let result of recipe_data.results) {
            for(let ending_quality = recipe_variable.ingredient_quality; ending_quality <= preferences.max_quality_unlocked; ending_quality++) {
                let result_item_key = get_item_key(result.name, ending_quality);
                ...
                let result_item_coefficients = item_variable_coefficients.get(result_item_key);
    ...
    preferences.inputs.forEach( (cost, input_item_key, map) => {
        let input_item_coefficients = item_variable_coefficients.get(input_item_key);
        let input_item_variable_key = get_input_item_variable_key(input_item_key);
...
export function get_variable_costs(recipe_variables, preferences) {
    ...
    preferences.inputs.forEach( (cost, item_key, map) => {
        let key = get_input_item_variable_key(item_key);
